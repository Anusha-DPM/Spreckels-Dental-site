...
export default function EditPost() {
  ...
  const loadPost = async () => {
    try {
      ...
      setFormData({
        ...
        blogPostingSchema: blogPost.blogPostingSchema || '',
        personSchema: blogPost.personSchema || '',
        dentistSchema: blogPost.dentistSchema || '',
        ...
        howToSchema: blogPost.howToSchema || '',
        customCode: blogPost.customCode || '',
        ogTitle: blogPost.ogTitle || '',
        ogDescription: blogPost.ogDescription || '',
        ogUrl: blogPost.ogUrl || '',
        twitterCard: blogPost.twitterCard || 'summary_large_image',
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      updateData.sitemapEntry = sitemapEntry
      updateData.canonicalUrl = getBlogCanonicalUrl(savedSlug, formData.canonicalUrl)
      ...
      updateData.jsonLdSchema = formData.jsonLdSchema?.trim() || ''
      updateData.blogPostingSchema = formData.blogPostingSchema?.trim() || ''
      updateData.personSchema = formData.personSchema?.trim() || ''
      updateData.dentistSchema = formData.dentistSchema?.trim() || ''
      updateData.breadcrumbActive = formData.breadcrumbActive?.trim() || ''
      updateData.faqSchema = formData.faqSchema?.trim() || ''
      updateData.medicalConditionSchema = formData.medicalConditionSchema?.trim() || ''
      updateData.howToSchema = formData.howToSchema?.trim() || ''

      // Always persist custom code + social meta so clearing fields works
      updateData.customCode = formData.customCode?.trim() || '';
      updateData.ogTitle = formData.ogTitle?.trim() || '';
      updateData.ogDescription = formData.ogDescription?.trim() || '';
      updateData.ogUrl = formData.ogUrl?.trim() || sitemapEntry;
      updateData.twitterCard = formData.twitterCard?.trim() || 'summary_large_image';
      updateData.twitterTitle = formData.twitterTitle?.trim() || '';
      updateData.twitterDescription = formData.twitterDescription?.trim() || '';
    ...
    <div className="min-h-screen bg-gray-50">
      ...
      <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        >
          ...
          <div className="bg-gray-50 p-6 rounded-lg">
            ...
            <div className="space-y-4">
              ...
              <div className="border-t border-gray-200 pt-4 mt-2">
                <h4 className="text-sm font-semibold text-gray-900 mb-3">Open Graph &amp; Twitter</h4>
                <div className="space-y-4">
                  <div>
                  ...
                  <div>
                    ...
                    <input
                      ...
                      suppressHydrationWarning
                      className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-[#441018] focus:border-transparent"
                      placeholder="Defaults to Canonical URL if empty"
                    />
                  </div>
                  <div>
          ...
          {/* Custom Code — injected only on this post's detail page */}
          <div className="bg-gray-50 p-6 rounded-lg">
            <h3 className="text-lg font-medium text-gray-900 mb-4">Custom Code</h3>
            <div>
              <label htmlFor="customCode" className="block text-sm font-medium text-gray-700 mb-2">
                HTML / CSS / JavaScript / JSON-LD
              </label>
              <textarea
                id="customCode"
                name="customCode"
                ...
                placeholder={'Paste HTML, <style>…</style>, <script>…</script>, or JSON-LD here.\nThis code appears only on this blog post.'}
              />
              <p className="text-xs text-gray-500 mt-1">
                Appears only on this blog post&apos;s page. You can paste markup with tags, or bare JSON / CSS / JS.
              </p>
            </div>
          </div>
