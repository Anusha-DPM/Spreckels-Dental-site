...
import { toAsciiSlug } from './sanitizeBlogHtml'
import {
  collection,
  addDoc,
  updateDoc,
...
export const getBlogPostBySlug = async (slug) => {
  try {
    ...
    // Fallback: match stored slugs after ASCII normalization so edited URLs still resolve
    const publishedPosts = await getPublishedBlogPosts()
    const matchedPost = publishedPosts.find((post) => {
      const storedSlug = typeof post.slug === 'string' ? post.slug.trim() : ''
      return storedSlug === requestedSlug || toAsciiSlug(storedSlug) === normalizedSlug
    })
