...
export const createBlogPost = async (blogData) => {
  try {
    ...
    existingPosts.unshift(localStoragePost)
    saveToLocalStorage(existingPosts)
    console.log('✅ Saved to localStorage with temp ID:', tempId)
    
    // Try to save to Firebase
    let firebaseId = tempId
    try {
      if (isFirebaseAvailable()) {
        ...
        if (index !== -1) {
          updatedPosts[index] = { ...updatedPosts[index], id: firebaseId }
          saveToLocalStorage(updatedPosts)
          console.log('✅ Updated localStorage with Firebase ID')
        }
      } else {
        console.warn('Firebase not available, using localStorage only')
      }
    } catch (firebaseError) {
      console.warn('Firebase save failed, using localStorage only:', firebaseError.message)
    }
    
    return { id: firebaseId, ...postData }
  } catch (error) {
