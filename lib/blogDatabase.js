...
export const getBlogPosts = async (options = {}) => {
  try {
    ...
    if (isFirebaseAvailable()) {
      ...
      // Filter out deleted posts before saving to localStorage
      ...
      // Only update localStorage if we're not in a delete operation
  ...
  const deletedPostIds = JSON.parse(sessionStorage.getItem('deletedPostIds') || '[]')
  let filteredPosts = posts.filter(post => !deletedPostIds.includes(post.id))
...
export const getBlogPostById = async (id) => {
  // Check if this post was deleted
  const deletedPostIds = JSON.parse(sessionStorage.getItem('deletedPostIds') || '[]')
  if (deletedPostIds.includes(id)) {
    console.log('⚠️ Post was deleted, not returning:', id)
    return null
  }
...
export const deleteBlogPost = async (id) => {
  try {
    // Set delete operation flag to prevent sync conflicts
    sessionStorage.setItem('deleteOperation', 'true')
    
    // Store the deleted post ID to prevent restoration
    ...
    // Clear delete operation flag after a delay
    setTimeout(() => {
    ...
    // Clear delete operation flag on error
    sessionStorage.removeItem('deleteOperation')
    throw error
  }
}
