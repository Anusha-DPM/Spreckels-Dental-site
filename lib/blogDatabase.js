...
import { normalizeBlogPostForDisplay } from './blogDisplayUtils'
import { extractBlogSlugFromUrl, toAsciiSlug } from './sanitizeBlogHtml'
import {
  collection,
  addDoc,
  updateDoc,
...
export const getBlogPostBySlug = async (slug) => {
  try {
    ...
    const normalizedSlug = toAsciiSlug(requestedSlug) || requestedSlug
    const slugsToTry = [...new Set([requestedSlug, normalizedSlug])]
    ...
    // Fallback: match stored slugs, canonical URLs, and sitemap entries
    const publishedPosts = await getPublishedBlogPosts()
    const matchedPost = publishedPosts.find((post) => {
      const storedSlug = typeof post.slug === 'string' ? post.slug.trim() : ''
      const aliasSlugs = [
        storedSlug,
        toAsciiSlug(storedSlug),
        extractBlogSlugFromUrl(post.canonicalUrl),
        extractBlogSlugFromUrl(post.sitemapEntry),
      ].filter(Boolean)

      return aliasSlugs.some(
        (alias) => alias === requestedSlug || toAsciiSlug(alias) === normalizedSlug
      )
    })

    if (matchedPost) {
      console.log('✅ Retrieved blog post by normalized slug:', matchedPost.title)
      return matchedPost
    }
...
export const getLatestBlogPosts = async (limit = 3) => {
  try {
    ...
    posts.sort((a, b) => new Date(b.publishDate).getTime() - new Date(a.publishDate).getTime())
    const limitedPosts = posts.slice(0, limit).map(normalizeBlogPostForDisplay)
