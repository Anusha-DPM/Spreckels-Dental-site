...
export const createBlogPost = async (postData) => {
  try {
    ...
    console.log('💾 Full post data being saved:', JSON.stringify({
      ...post,
      content: post.content?.substring(0, 100) + '...' // Truncate content for logging
    }, null, 2))
    ...
    if (savedDoc.exists()) {
      ...
      console.log('✅ Verified saved document:', {
        ...
        hasCoverImage: !!savedData.coverImage
      })
      if (!savedData.coverImage && post.coverImage) {
        console.error('⚠️ WARNING: coverImage was in post data but not found in saved document!')
      }
    }
...
export const updateBlogPost = async (id, postData) => {
  try {
    ...
    console.log('💾 Full update data:', JSON.stringify({
      ...updateData,
      content: updateData.content?.substring(0, 100) + '...' // Truncate content for logging
    }, null, 2))
    ...
    if (updatedDoc.exists()) {
      ...
      console.log('✅ Verified updated document:', {
        id: updatedDoc.id,
        coverImage: updatedData.coverImage,
        imageUrl: updatedData.imageUrl,
        hasCoverImage: !!updatedData.coverImage
      })
      if (!updatedData.coverImage && updateData.coverImage) {
        console.error('⚠️ WARNING: coverImage was in update data but not found in updated document!')
      }
    }
  } catch (error) {
