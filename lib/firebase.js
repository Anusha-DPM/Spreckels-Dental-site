...
export const uploadImageToFirebase = async (file, folder = 'blog-images', retryCount = 0) => {
  ...
  console.log(`🚀 Starting image upload process... (attempt ${retryCount + 1}/${maxRetries + 1})`);
  ...
  try {
    ...
    // Step 4: Upload file with timeout
    ...
    console.log('✅ File uploaded successfully to Firebase Storage:', fileName);
...
// Test small image upload specifically
      ...
      canvas.toBlob(async (blob) => {
        if (blob) {
          ...
          try {
            ...
            resolve({ success: true, message: 'Small image upload is working', result });
          } catch (error) {
