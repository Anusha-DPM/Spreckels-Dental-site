...
export const uploadImageToFirebase = async (file, folder = 'images', retryCount = 0) => {
  ...
  try {
    ...
    if (error.message.includes('Firebase Storage is not configured') || error.message.includes('Firebase Storage is not initialized')) {
      ...
      } else if (errorData?.initError) {
        errorMsg += ` Firebase initialization error: ${errorData.initError}.`;
        errorMsg += ' Please check your Firebase configuration values in .env.local and ensure they are correct.';
        errorMsg += ' Make sure you have restarted your Next.js development server after updating .env.local.';
      } else {
        errorMsg += ' Please check your environment variables (NEXT_PUBLIC_FIREBASE_*).';
        errorMsg += ' IMPORTANT: Make sure you have restarted your Next.js development server after updating .env.local file.';
      }
