...
export const uploadImageToFirebase = async (file, folder = 'images') => {
  ...
  try {
    ...
    if (!response.ok) {
      ...
      // Check if error indicates Firebase Storage is required (e.g., on Vercel)
      const requiresFirebaseOnly = errorData.requiresFirebaseOnly || 
                                   errorData.error?.includes('Vercel') ||
                                   errorData.details?.includes('Vercel') ||
                                   errorData.error?.includes('environment variables in Vercel');
      
      // Check if we're on Vercel (client-side detection)
      ...
      let errorMessage = errorData.error || errorData.details || `Upload failed with status ${response.status}`;
      
      // If on Vercel or Firebase is required, provide specific guidance
      if (isVercel || requiresFirebaseOnly) {
        const missingVars = errorData.missingEnvVars || [];
        if (missingVars.length > 0) {
          errorMessage = `Firebase Storage is not configured on Vercel. Missing environment variables: ${missingVars.join(', ')}.\n\nTo fix this:\n1. Go to your Vercel project → Settings → Environment Variables\n2. Add all NEXT_PUBLIC_FIREBASE_* variables\n3. Enable them for Production, Preview, and Development\n4. Redeploy your application\n\nSee VERCEL_FIREBASE_SETUP.md for detailed instructions.`;
        } else {
          errorMessage = `Firebase Storage is not configured on Vercel. Please add all NEXT_PUBLIC_FIREBASE_* environment variables in Vercel project settings → Environment Variables. After adding variables, redeploy your application. Local file storage is not available on Vercel.`;
        }
      }
      ...
      console.error('❌ Upload API error response:', {
        ...
        error: errorData,
        isVercel: isVercel,
        requiresFirebaseOnly: requiresFirebaseOnly
      });
      
      // Create error with additional details
      const error = new Error(errorMessage);
      error.errorData = errorData; // Attach error data for better error handling
      error.requiresFirebaseOnly = requiresFirebaseOnly || isVercel; // Flag to skip local storage fallback
      throw error;
    }
    ...
    // Check if error explicitly says Firebase is required (e.g., from API route on Vercel)
    const requiresFirebaseOnly = error.requiresFirebaseOnly || 
                                 error.errorData?.requiresFirebaseOnly ||
                                 error.message.includes('Vercel') ||
                                 error.message.includes('environment variables in Vercel');
    
    // Check if we're on Vercel (serverless environment - no local file storage)
    // Check multiple ways to detect Vercel since process.env might not be available client-side
    const isVercel = typeof window !== 'undefined' && (
      window.location.hostname.includes('vercel.app') ||
      window.location.hostname.includes('vercel.com')
    ...
    // Only fallback to local storage if NOT on Vercel AND Firebase-only is not required
    if (isFirebaseStorageError && !isVercel && !requiresFirebaseOnly) {
      ...
      try {
        ...
        // Use the original file, not the compressed one, for the fallback
        ...
        // Check if we're on Vercel (fallback shouldn't have been attempted, but just in case)
        const isVercelNow = typeof window !== 'undefined' && (
          window.location.hostname.includes('vercel.app') ||
          window.location.hostname.includes('vercel.com')
        );
        
        if (isVercelNow) {
          throw new Error(`Upload failed: Firebase Storage is not configured on Vercel. Please add all NEXT_PUBLIC_FIREBASE_* environment variables in Vercel project settings → Environment Variables. After adding variables, redeploy your application. Local file storage is not available on Vercel.`);
        }
        
        // Provide more helpful error message with full details
        const fallbackErrorMsg = fallbackError.message || fallbackError.toString() || 'Unknown error';
        throw new Error(`Upload failed: Firebase Storage is not available, and local storage also failed. ${fallbackErrorMsg}`);
      }
    } else if (isFirebaseStorageError && (isVercel || requiresFirebaseOnly)) {
      // On Vercel or when Firebase-only is required, Firebase Storage is required (no local file storage available)
      console.error('❌ Firebase Storage is required. Local file storage is not available on this platform.');
      if (isVercel) {
        throw new Error(`Upload failed: Firebase Storage is not configured. Please set all NEXT_PUBLIC_FIREBASE_* environment variables in Vercel. Visit your Vercel project settings → Environment Variables to configure Firebase. After adding variables, redeploy your application.`);
      } else {
        throw new Error(`Upload failed: Firebase Storage is not configured. ${error.message || 'Please configure Firebase Storage to upload images.'}`);
      }
    }
