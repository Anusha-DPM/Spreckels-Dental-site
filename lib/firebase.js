...
export const uploadImageToFirebase = async (file, folder = 'blog-images', retryCount = 0) => {
  const maxRetries = 2;
...
// Test Firebase Storage connection via server-side API
export const testFirebaseStorage = async () => {
  console.log('🧪 Testing server-side Firebase Storage connection...');

  try {
    // Create a small test file
    const testContent = 'Firebase Storage test - ' + new Date().toISOString();
    const testBlob = new Blob([testContent], { type: 'text/plain' });
    const testFile = new File([testBlob], 'test-connection.txt', { type: 'text/plain' });
    
    console.log('📤 Testing server-side upload...');
    
    // Test via server-side API
    const formData = new FormData();
    ...
    const result = await response.json();
    console.log('✅ Server-side test upload successful:', result);
    
    return { success: true, message: 'Server-side Firebase Storage is working correctly', result };
  } catch (error) {
...
// Test small image upload specifically via server-side API
export const testSmallImageUpload = async () => {
  console.log('🧪 Testing small image upload via server-side API...');
  ...
  try {
    ...
    return new Promise((resolve) => {
      canvas.toBlob(async (blob) => {
        if (blob) {
          const testFile = new File([blob], 'test-image-tiny.png', { type: 'image/png' });
          console.log('📊 Test file size:', testFile.size, 'bytes');
          
          try {
            const result = await uploadImageToFirebase(testFile, 'test-uploads');
            console.log('✅ Small image upload test successful:', result);
            resolve({ success: true, message: 'Small image upload is working via server-side API', result });
          } catch (error) {
            console.error('❌ Small image upload test failed:', error);
            resolve({ success: false, error: error.message });
          }
        } else {
          resolve({ success: false, error: 'Failed to create test image blob' });
        }
      }, 'image/png');
    });
    
  } catch (error) {
    console.error('❌ Small image test failed:', error);
    return { success: false, error: error.message };
  }
};
