...
export const uploadImageToFirebase = async (file, folder = 'images') => {
  ...
  try {
    ...
    // Only fallback to local storage if NOT on Vercel (Vercel has read-only filesystem)
    if (isFirebaseStorageError && !isVercel) {
    ...
    } else if (isFirebaseStorageError && isVercel) {
      // On Vercel, Firebase Storage is required (no local file storage available)
      console.error('❌ Firebase Storage is required on Vercel. Local file storage is not available.');
      throw new Error(`Upload failed: Firebase Storage is not configured. Please set all NEXT_PUBLIC_FIREBASE_* environment variables in Vercel. Visit your Vercel project settings → Environment Variables to configure Firebase.`);
    }
